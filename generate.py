...
cjpeg = "~/encoding/mozjpeg/build/cjpeg-static -quality 99 -progressive"
outfolder = "images"
...
draw.text((128 * scale, 64 * scale), str(int(time.time())), fill=(0, 0, 0), font=font3)

draw.text((h, 1280 * scale + h), "R", fill=(255, 0, 0), font=font)
draw.text((h + font.getlength("R"), 1280 * scale + h), "G", fill=(0, 255, 0), font=font)
draw.text(
    (h + font.getlength("RG"), 1280 * scale + h), "B", fill=(0, 0, 255), font=font
)

alpha = Image.new("RGBA", (w, w), (0, 0, 0, 0))
drawa = ImageDraw.Draw(alpha)
drawa.text(
    (h + font.getlength("RGB"), 1280 * scale + h),
    "A",
...
draw.text(
    (2048 * scale + font.getlength("C"), 1280 * scale + h),
    "M",
    fill=(255, 0, 255),
    font=font,
)
draw.text(
    (2048 * scale + font.getlength("CM"), 1280 * scale + h),
    "Y",
    fill=(255, 255, 0),
    font=font,
)
draw.text(
    (2048 * scale + font.getlength("CMY"), 1280 * scale + h),
    "K",
    fill=(0, 0, 0),
    font=font,
)
...
for i, fs in enumerate(formats, 0):
    ...
    if cmd:
        ...
        if subprocess.run(cmdr, shell=True).returncode != 0:
            exit(1)
        if not keep:
